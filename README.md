...
A sales-demo benchmark suite for the **Dell Pro Max GB10 (Grace-Blackwell, aarch64)**.
...
- **Precision sweep** — FP4 / NVFP4 / FP8 / INT8 / FP16 / BF16 / FP32 via bitsandbytes +
  native dtypes, with honest notes on what the Dell Pro Max GB10's HF stack actually computes (e.g. NVFP4
...
- **TCO engine** (`webui/helpers/tco_engine.py`) — Dell PowerEdge XE catalog, per-GPU
  ...
  - **LLM Inference** — per-user tok/s + $/MTok, anchored on *measured* Dell Pro Max GB10 throughput.
...
- **On-device AI narrator** (`webui/helpers/on_device_ai.py`) — a local instruct model
  that narrates results and the TCO recommendation *on the Dell Pro Max GB10*, no cloud.
...
- Ubuntu 24.04 LTS **aarch64**, **Dell Pro Max GB10**, CUDA 13.0
