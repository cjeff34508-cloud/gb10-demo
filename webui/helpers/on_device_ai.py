"""
On-device AI narration — runs a local instruct model ON THE GB10 to generate
...
The narrator is kept RESIDENT on the GB10 by default (see streamlit_app's
...
class OnDeviceNarrator:
    """Loads the narrator model on the GB10, streams analysis, then unloads."""
    ...
    def results_summary(self, rdata: dict) -> Iterator[str]:
        system = (
            "You are a Dell + NVIDIA solutions engineer presenting GB10 (Grace-Blackwell, "
    ...
    # -- cloud comparison (on-prem GB10 vs hosted Claude/Gemini APIs) ------------
    ...
    def explain_model(self, name: str, scenario: str, curated_body: str = "") -> Iterator[str]:
        system = (
            ...
            "audience evaluating the NVIDIA GB10 (Grace-Blackwell, 128 GB unified LPDDR5X "
        ...
        user = (
            f"Give a detailed deep-dive on '{name}' as used in the GB10 scenario '{scenario}'. "
            "Cover: architecture & params, what it's good at, compute-bound vs "
            "memory-bandwidth-bound on the GB10 and why, precision behavior "
